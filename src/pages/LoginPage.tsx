...
export default function LoginPage({ onSwitchToRegister }: LoginPageProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white dark:from-gray-950 dark:to-gray-900">
        ...
        <div className="grid grid-cols-2 gap-16 items-center">
          ...
          <div>
            ...
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4 tracking-tight leading-tight">
              让每一天都<br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-orange-500">高效有序</span>
            </h1>
            <p className="text-base text-gray-500 dark:text-gray-400 mb-8 leading-relaxed">
              集任务管理、番茄钟、AI 智能整理于一体的工作台。<br />
              无论是个人待办还是团队协作，都能轻松应对。
            </p>
            {/* 功能亮点 */}
            <div className="grid grid-cols-2 gap-3">
              {[
                { icon: CheckSquare, text: '三视图任务管理', color: 'text-blue-500' },
                { icon: Timer, text: '番茄钟专注计时', color: 'text-red-500' },
                { icon: Sparkles, text: 'AI 智能助手', color: 'text-purple-500' },
                { icon: Users, text: '团队实时协作', color: 'text-green-500' },
              ].map((f, i) => (
          ...
          <div>
            <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl shadow-gray-200/50 dark:shadow-black/20 border border-gray-100 dark:border-gray-800 p-8">
              ...
              <p className="text-sm text-gray-400 mb-6">欢迎回来，请输入您的账号</p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-2.5 bg-gradient-to-r from-red-500 to-orange-500 text-white rounded-lg text-sm font-medium hover:opacity-90 transition-opacity disabled:opacity-50 shadow-lg shadow-red-500/20"
                >
                  {loading ? '登录中...' : '登录'}
                </button>
              </form>
              ...
              <p className="mt-5 text-center text-xs text-gray-400">
                还没有账号？
                <button onClick={onSwitchToRegister} className="ml-1 text-red-500 hover:text-red-600 font-medium">注册</button>
              </p>
            </div>
          </div>
        </div>
      ...
      {/* 功能卡片 */}
      <section id="features" className="max-w-6xl mx-auto px-8 pb-20">
        <h2 className="text-2xl font-bold text-center text-gray-900 dark:text-white mb-2">核心功能</h2>
        <p className="text-sm text-gray-400 text-center mb-10">一站式解决你的效率需求</p>
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { icon: Shield, title: '安全可靠', desc: 'JWT 认证、数据加密、定时备份', color: 'from-slate-500 to-gray-600' },
          ].map((f, i) => (
            <div key={i} className="group p-5 rounded-xl bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 hover:border-gray-200 dark:hover:border-gray-700 hover:shadow-md transition-all duration-200">
              <div className={`w-9 h-9 rounded-lg bg-gradient-to-br ${f.color} flex items-center justify-center mb-3 group-hover:scale-110 transition-transform`}>
      ...
      <section id="pricing" className="max-w-6xl mx-auto px-8 pb-20">
        <h2 className="text-2xl font-bold text-center text-gray-900 dark:text-white mb-2">选择适合你的方案</h2>
        <p className="text-sm text-gray-400 text-center mb-10">灵活定价，按需选择</p>
        <div className="grid grid-cols-2 gap-6 max-w-2xl mx-auto">
          ...
          <div className="p-6 rounded-2xl bg-gradient-to-br from-gray-900 to-gray-800 dark:from-white dark:to-gray-100 text-white dark:text-gray-900 relative overflow-hidden">
            <div className="absolute top-4 right-4 px-2 py-0.5 text-[10px] font-medium bg-white/20 dark:bg-gray-900/10 rounded-full">推荐</div>
